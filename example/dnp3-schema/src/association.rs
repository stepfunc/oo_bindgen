...
pub fn define(lib: &mut LibraryBuilder, association_class: ClassDeclarationHandle) -> Result<(), BindingError> {
    ...
    // Command stuff
    let command_mode = lib.define_native_enum("CommandMode")?
        .push("DirectOperate")?
        .push("SelectBeforeOperate")?
        .build();

    let trip_close_code = lib.define_native_enum("TripCloseCode")?
        .variant("Nul", 0)?
        .variant("Close", 1)?
        .variant("Trip", 2)?
        .variant("Reserved", 3)?
    ...
    let op_type = lib.define_native_enum("OpType")?
        .variant("Nul", 0)?
        .variant("PulseOn", 1)?
        .variant("PulseOff", 2)?
        .variant("LatchOn", 3)?
    ...
    let control_code = lib.define_native_struct(&control_code)?
    ...
    let g12v1_struct = lib.define_native_struct(&g12v1_struct)?
    ...
    let command_new_fn = lib.declare_native_function("command_new")?
        .return_type(ReturnType::Type(Type::ClassRef(command.clone())))?
        .build()?;

    let command_destroy_fn = lib.declare_native_function("command_destroy")?
        .param("command", Type::ClassRef(command.clone()))?
        .return_type(ReturnType::Void)?
        .build()?;

    let command_add_u8_g12v1_fn = lib.declare_native_function("command_add_u8_g12v1")?
        .param("command", Type::ClassRef(command.clone()))?
    ...
    let command_add_u16_g12v1_fn = lib.declare_native_function("command_add_u16_g12v1")?
        .param("command", Type::ClassRef(command.clone()))?
    ...
    let command_add_u8_g41v1_fn = lib.declare_native_function("command_add_u8_g41v1")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint8)?
    ...
    let command_add_u16_g41v1_fn = lib.declare_native_function("command_add_u16_g41v1")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint16)?
        .param("value", Type::Sint32)?
        .return_type(ReturnType::Void)?
    ...
    let command_add_u8_g41v2_fn = lib.declare_native_function("command_add_u8_g41v2")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint8)?
    ...
    let command_add_u16_g41v2_fn = lib.declare_native_function("command_add_u16_g41v2")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint16)?
        .param("value", Type::Sint16)?
        .return_type(ReturnType::Void)?
    ...
    let command_add_u8_g41v3_fn = lib.declare_native_function("command_add_u8_g41v3")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint8)?
    ...
    let command_add_u16_g41v3_fn = lib.declare_native_function("command_add_u16_g41v3")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint16)?
        .param("value", Type::Float)?
        .return_type(ReturnType::Void)?
    ...
    let command_add_u8_g41v4_fn = lib.declare_native_function("command_add_u8_g41v4")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint8)?
    ...
    let command_add_u16_g41v4_fn = lib.declare_native_function("command_add_u16_g41v4")?
        .param("command", Type::ClassRef(command.clone()))?
        .param("idx", Type::Uint16)?
        .param("value", Type::Double)?
        .return_type(ReturnType::Void)?
    ...
    let command = lib.define_class(&command)?
    ...
    let command_result = lib.define_native_enum("CommandResult")?
        .push("Success")?
        .push("TaskError")?
        .push("BadStatus")?
    ...
    let command_cb = lib.define_one_time_callback("CommandTaskCallback")?
    ...
    let operate_fn = lib.declare_native_function("association_operate")?
        .param("association", Type::ClassRef(association_class.clone()))?
