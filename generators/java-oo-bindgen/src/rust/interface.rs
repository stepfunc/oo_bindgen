...
pub(crate) fn generate_interfaces_cache(
    ...
    for interface in lib.interfaces() {
        ...
        blocked(&mut f, |f| {
            write_interface_init(f, &interface_name, &lib_path, interface.callbacks().collect())?;

            f.newline()?;

            let rust_struct_name = format!("{}::ffi::{}", config.ffi_name, interface.name.to_camel_case());
                    ...
                    for element in &interface.elements {
                        match element {
                            InterfaceElement::CallbackFunction(callback) => {
                                f.writeln(&format!("{}: Some({}_{}),", callback.name.to_snake_case(), interface.name.to_camel_case(), callback.name.to_snake_case()))?;
                            },
                            ...
                            InterfaceElement::DestroyFunction(name) => {
                                f.writeln(&format!("{}: Some({}_{}),", name.to_snake_case(), interface.name.to_camel_case(), name.to_snake_case()))?;
                            }
                        }
                    }
                    Ok(())
        ...
        for element in &interface.elements {
            match element {
                InterfaceElement::CallbackFunction(callback) => {
                    let params = callback.parameters.iter().map(|param| {
                        match param {
                            CallbackParameter::Parameter(param) => format!("{}: {}", param.name.to_snake_case(), param.param_type.as_rust_type(&config.ffi_name)),
                            CallbackParameter::Arg(name) => format!("{}: *mut std::ffi::c_void", name),
                        }
                    }).collect::<Vec<_>>().join(", ");
                    f.writeln("#[allow(non_snake_case)]")?;
                    f.writeln(&format!("extern \"C\" fn {}_{}({}) -> {}", interface.name.to_camel_case(), callback.name.to_snake_case(), params, callback.return_type.as_rust_type(&config.ffi_name)))?;
                    blocked(&mut f, |f| {
                        call_java_callback(f, &format!("interface_{}", interface_name.to_snake_case()), &callback.name.to_snake_case(), &lib_path, &config.ffi_name, &callback.arg_name, callback.params().collect(), &callback.return_type)?;
                        ...
                        if let ReturnType::Type(return_type, _) = &callback.return_type {
                            if let Some(conversion) = return_type.conversion(&config.ffi_name) {
                                conversion.convert_to_rust(f, &format!("_result.{}", return_type.convert_jvalue()), "return ")?;
                                f.write(";")?;
                            } else {
                ...
                InterfaceElement::DestroyFunction(name) => {
                    f.writeln("#[allow(non_snake_case)]")?;
                    f.writeln(&format!("extern \"C\" fn {}_{}(ctx: *mut std::ffi::c_void)", interface.name.to_camel_case(), name.to_snake_case()))?;
                    blocked(&mut f, |f| {
    ...
    // Each one-time callback implementation
    for interface in lib.one_time_callbacks() {
        let interface_name = interface.name.to_camel_case();

        f.writeln(&format!("pub struct Callback{}", interface_name))?;
        blocked(&mut f, |f| {
            f.writeln("_class: jni::objects::GlobalRef,")?;
            for callback in interface.callbacks() {
                f.writeln(&format!(
        ...
        f.writeln(&format!("impl Callback{}", interface_name))?;
        blocked(&mut f, |f| {
            write_interface_init(f, &interface_name, &lib_path, interface.callbacks().collect())?;
            ...
            blocked(f, |f| {
                ...
                blocked(f, |f| {
                    for element in &interface.elements {
                        match element {
                            OneTimeCallbackElement::CallbackFunction(callback) => {
                                f.writeln(&format!("{}: Some({}_{}),", callback.name.to_snake_case(), interface.name.to_camel_case(), callback.name.to_snake_case()))?;
                            },
        ...
        for element in &interface.elements {
            match element {
                OneTimeCallbackElement::CallbackFunction(callback) => {
                    ...
                    f.writeln(&format!("extern \"C\" fn {}_{}({}) -> {}", interface.name.to_camel_case(), callback.name.to_snake_case(), params, callback.return_type.as_rust_type(&config.ffi_name)))?;
                    blocked(&mut f, |f| {
                        call_java_callback(f, &format!("callback_{}", interface_name.to_snake_case()), &callback.name.to_snake_case(), &lib_path, &config.ffi_name, &callback.arg_name, callback.params().collect(), &callback.return_type)?;

                        // Convert return value
                        if let ReturnType::Type(return_type, _) = &callback.return_type {
                            if let Some(conversion) = return_type.conversion(&config.ffi_name) {
                                conversion.convert_to_rust(f, &format!("_result.{}", return_type.convert_jvalue()), "let _result = ")?;
                                f.write(";")?;
                            }
                        }

                        // Consume the inner pointer
                        f.writeln(&format!("unsafe {{ Box::from_raw({} as *mut jni::objects::GlobalRef) }};", callback.arg_name))?;
...
fn write_interface_init<'a>(f: &mut dyn Printer, interface_name: &str, lib_path: &str, callbacks: Vec<&'a CallbackFunction>) -> FormattingResult<()> {
    f.writeln("pub fn init(env: &jni::JNIEnv) -> Self")?;
    blocked(f, |f| {
        f.writeln(&format!(
            "let class = env.find_class(\"L{}/{};\").expect(\"Unable to find {}\");",
            lib_path, interface_name.to_camel_case(), interface_name
        ))?;
        for callback in &callbacks {
            let method_sig = format!("({}){}", callback.params().map(|param| {
                param.param_type.as_jni_sig(&lib_path)
            }).collect::<Vec<_>>().join(", "), callback.return_type.as_jni_sig(&lib_path));
...
fn call_java_callback<'a>(f: &mut dyn Printer, interface_name: &str, callback_name: &str, lib_path: &str, ffi_name: &str, arg_name: &str, params: Vec<&'a Parameter>, return_type: &ReturnType) -> FormattingResult<()> {
    // Extract the global ref
    f.writeln(&format!("let _obj = unsafe {{ &mut *({} as *mut jni::objects::GlobalRef) }};", arg_name))?;
    ...
    for param in params {
        if param.param_type.as_raw_jni_type() == "jni::sys::jobject" {
            f.writeln(&format!("_env.delete_local_ref({}.into()).unwrap();", param.name.to_snake_case()))?;
        }
    }
