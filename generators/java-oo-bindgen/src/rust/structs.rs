...
pub(crate) fn generate_structs_cache(
    ...
    for structure in lib.structs() {
        ...
        blocked(&mut f, |f| {
            ...
            blocked(f, |f| {
                for field in structure.fields() {
                    f.writeln(&format!("let temp = _env.get_field_unchecked(obj, self.field_{}, jni::signature::JavaType::from_str(\"{}\").unwrap()).unwrap().{};", field.name.to_snake_case(), field.field_type.to_all_types().as_jni_sig(&lib_path), field.field_type.to_all_types().convert_jvalue()))?;
                    field.field_type.to_all_types().check_null(f, "temp")?;
                }
                f.writeln("Ok(())")
            })
        })?;
