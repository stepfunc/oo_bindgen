    ...
    for structure in lib.structs() {
        ...
        blocked(&mut f, |f| {
            ...
            blocked(f, |f| {
                ...
                for field in structure.fields() {
                    f.writeln(&format!("let field_{field_snake} = env.get_field_id(class, \"{field_mixed}\", \"{field_sig}\").map(|mid| mid.into_inner().into()).expect(\"Unable to find field {field_mixed}\");", field_snake=field.name.to_snake_case(), field_mixed=field.name.to_mixed_case(), field_sig=field.field_type.to_any_type().as_jni_sig(&lib_path)))?;
                }
                f.writeln("Self")?;
                blocked(f, |f| {
            ...
            blocked(f, |f| {
                for field in structure.fields() {
                    f.writeln(&format!("let {} = _env.get_field_unchecked(obj, self.field_{}, jni::signature::JavaType::from_str(\"{}\").unwrap()).unwrap().{};", field.name.to_snake_case(), field.name.to_snake_case(), field.field_type.to_any_type().as_jni_sig(&lib_path), field.field_type.to_any_type().convert_jvalue()))?;
                }
            ...
            blocked(f, |f| {
                ...
                for field in structure.fields() {
                    ...
                    if field.field_type.to_any_type().requires_local_ref_cleanup() {
            ...
            blocked(f, |f| {
                for field in structure.fields() {
                    f.writeln(&format!("let temp = _env.get_field_unchecked(obj, self.field_{}, jni::signature::JavaType::from_str(\"{}\").unwrap()).unwrap().{};", field.name.to_snake_case(), field.field_type.to_any_type().as_jni_sig(&lib_path), field.field_type.to_any_type().convert_jvalue()))?;
                    field.field_type.to_any_type().check_null(f, "temp")?;
                }
                f.writeln("Ok(())")
            })
        })?;
