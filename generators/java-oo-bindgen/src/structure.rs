...
pub(crate) fn generate(
    ...
    blocked(f, |f| {
        ...
        // Write native struct
        let field_order = native_struct.elements()
            .map(|el| {
                format!("\"{}\"", el.name.to_mixed_case())
            })
            .collect::<Vec<_>>()
            .join(", ");
        ...
        blocked(f, |f| {
            ...
            blocked(f, |f| {
                ...
                f.writeln(&format!("public ByValue({} self) {{ super(self); }}", struct_name))
            })?;
            ...
            blocked(f, |f| {
                ...
                f.writeln(&format!("public ByReference({} self) {{ super(self); }}", struct_name))
            })?;
