...
pub fn define(lib: &mut LibraryBuilder) -> Result<(), BindingError> {
    ...
    let iterator_next_fn = lib.declare_native_function("iterator_next")?
    ...
    let iterator_item = lib.define_native_struct(&iterator_item)?
    ...
    // Define test method
    let iterate_string_fn = lib.declare_native_function("iterator_create")?
        .param("value", Type::String)?
        .return_type(ReturnType::Type(Type::Iterator(iterator)))?
        .build()?;
    let iterator_destroy_fn = lib.declare_native_function("iterator_destroy")?
        .param("it", Type::ClassRef(iterator_class.clone()))?
